...
!!! info
The style guide is always a work in progress, and not all EquationsOfStateOfSolids code
follows the rules. When modifying EquationsOfStateOfSolids, please fix the style violations
...
`EquationsOfStateOfSolids` uses [`JuliaFormatter`](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/MineralsCloud/EquationsOfStateOfSolids.jl/blob/master/.JuliaFormatter.toml).

To format your code, `cd` to the `EquationsOfStateOfSolids` directory, then run:
...
!!! info
A continuous integration check verifies that all PRs made to `EquationsOfStateOfSolids` have
